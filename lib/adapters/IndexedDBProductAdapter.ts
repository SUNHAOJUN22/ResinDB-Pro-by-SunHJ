...
import { Category, Product, ProductUpdates, PropertyValue } from "../../types";
import { PRODUCT_CATALOG, CATEGORY_TREE } from "../../constants";
...
export class IndexedDBProductAdapter implements IProductAdapter {
  ...
  private async simulateLatency(ms?: number): Promise<void> {
    ...
    // @ts-expect-error - import.meta.env is available in Vite but TS might complain
  ...
  async create(product: Partial<Product>): Promise<Product> {
    ...
    try {
      ...
      console.error("IndexedDB Create Failed:", e);
  ...
  async batchCreate(products: Partial<Product>[]): Promise<Product[]> {
    ...
    for (const p of products) {
      ...
      try {
        ...
        console.warn(`Skipping product due to error during batch export: ${newProduct.id}`, err);
