...
- 已知存在安全问题且没有维护修复的浏览器端 XLSX 解析依赖。

关键行为也已收敛：刷新失败不再显示成功或覆盖当前数据；并发写入失败只回滚受影响记录；沙箱不再声称执行外部软件或连接真实仪器。
...
XLS/XLSX 不在浏览器端直接解析。先由可信办公软件导出为 CSV，可减少公式、宏、隐藏单元格和第三方解析器带来的风险。
