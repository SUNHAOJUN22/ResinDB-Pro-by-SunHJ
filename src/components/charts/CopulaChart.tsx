...
import { SCIENTIFIC_SEQUENTIAL, formatScientificNumber } from './scientificFigurePolicy';
...
export const CopulaChart: React.FC<CopulaChartProps> = React.memo(({ grid, theme }) => {
  const option = useMemo<EChartsOption>(() => {
    ...
    return {
      ...
      tooltip: { formatter: (params: { data?: unknown }) => { const value = params.data as [number, number, number] | undefined; return value ? `u: ${formatScientificNumber(value[0])}<br/>v: ${formatScientificNumber(value[1])}<br/>Fitted density: ${formatScientificNumber(value[2])}` : ''; } },
