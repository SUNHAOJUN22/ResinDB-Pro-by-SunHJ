...
function getDirectFeatureValue(product: Product, key: string): number | null {
  return parseFinite(product.properties?.[key]?.value);
}

function getAliasedValue(product: Product, keys: readonly string[]): number | null {
...
/**
 * This preserves the Phase 2K formula and rule definitions. The migration only
 * makes unavailable evidence explicit instead of rendering it as a numeric zero.
 */
export function evaluateDependencyOutput(
  ...
  const density = Math.max(0, getAliasedValue(product, ['密度', 'density', 'Density']) ?? 0.95);
  const mfr = getAliasedValue(product, ['熔体质量流动速率', 'mfr', 'MFR']) ?? 2;
  const tensile = getAliasedValue(product, [
    '拉伸屈服应力',
    '拉伸断裂应力',
    'tensileYield',
    'tensile',
  ...
  const flexural = getAliasedValue(product, [
    '弯曲模量',
    'flexuralModulus',
    '弯曲弹性模量',
    'Flexural Modulus',
  ]) ?? 1200;
  const impact = getAliasedValue(product, [
    '简支梁缺口冲击强度',
    '悬臂梁缺口冲击强度',
    'izodImpact',
    '冲击强度',
  ...
  switch (colKey) {
    ...
    case 'toughness': {
      const modulusFactor = flexural > 0 ? Math.sqrt(1200 / flexural) : 1;
      return impact * modulusFactor;
    }
    default:
      return null;
  }
...
export function buildDependencyProperties(
  ...
  return [
    ...
    },
    {
      ...
      descriptionEn: 'Screening proxy generated by the existing reciprocal MFR rule; not measured rheology.',
    ...
    },
    {
      ...
      descriptionEn: 'Screening proxy generated by the existing tensile-density rule.',
      evidenceType: 'rule-generated-proxy',
    },
    {
      key: 'stiffness',
      ...
      descriptionEn: 'Screening proxy generated by the existing flexural-density rule.',
      evidenceType: 'rule-generated-proxy',
    },
    {
      key: 'toughness',
      ...
      descriptionEn: 'Screening proxy generated by the existing impact-modulus rule.',
      evidenceType: 'rule-generated-proxy',
    },
  ];
}

function perturbProduct(product: Product, key: string, value: number): Product {
  return {
    ...product,
    properties: {
      ...product.properties,
...
export function computeDependencySensitivityCells(
  ...
  return rows.flatMap((row) => {
    const baseInput = getDirectFeatureValue(referenceProduct, row.key);
    if (baseInput === null || baseInput === 0) {
      return columns.map((column) => ({
        rowKey: row.key,
        colKey: column.key,
        score: null,
        availability: 'missing-input' as const,
    ...
    return columns.map((column) => {
      ...
      if (baseOutput === null || baseOutput === 0) {
        return {
          ...
          colKey: column.key,
          score: null,
          availability: 'unavailable-output' as const,
          evidenceType: column.evidenceType,
      ...
      const plusOutput = evaluateDependencyOutput(
        column.key,
        perturbProduct(referenceProduct, row.key, plusInput),
        expression,
        formulas,
      );
      const minusOutput = evaluateDependencyOutput(
        column.key,
        perturbProduct(referenceProduct, row.key, minusInput),
        expression,
        formulas,
      );
      ...
      return Number.isFinite(sensitivity)
        ? {
            rowKey: row.key,
            colKey: column.key,
...
export function buildDependencyCurve(
  ...
  if (!referenceProduct) return [];
  const baseInput = getDirectFeatureValue(referenceProduct, rowKey);
  if (baseInput === null || baseInput === 0) return [];

  return [-40, -30, -20, -10, 0, 10, 20, 30, 40].flatMap((percent) => {
    const value = evaluateDependencyOutput(
      colKey,
      perturbProduct(referenceProduct, rowKey, baseInput * (1 + percent / 100)),
      expression,
      formulas,
    );
    return value === null
...
function evidenceLabel(evidenceType: DependencyEvidenceType, language: 'zh' | 'en'): string {
  if (evidenceType === 'formula-dependency') {
    return localize(language, '公式依赖', 'formula dependency');
  }
  return localize(language, '规则生成代理', 'rule-generated proxy');
}

function availabilityLabel(availability: DependencyAvailability, language: 'zh' | 'en'): string {
  if (availability === 'missing-input') return localize(language, '输入缺失', 'missing input');
  if (availability === 'unavailable-output') return localize(language, '输出不可用', 'unavailable output');
...
export function getDependencyHeatmapOption(
  ...
  const maximumMagnitude = Math.max(
    1,
    ...cells.map((cell) => cell.score === null ? 0 : Math.abs(cell.score)),
  );
  const data: DependencyHeatmapPoint[] = cells.map((cell) => {
    const rowIndex = rows.findIndex((row) => row.key === cell.rowKey);
    const columnIndex = columns.findIndex((column) => column.key === cell.colKey);
    const row = rows[rowIndex];
    const column = columns[columnIndex];
    const isSelected = selected?.rowKey === cell.rowKey && selected.colKey === cell.colKey;
    return {
      value: [columnIndex, rowIndex, cell.score === null ? 0 : Math.abs(cell.score)],
      rowKey: cell.rowKey,
      colKey: cell.colKey,
      inputLabel: row ? localize(language, row.labelZh, row.labelEn) : cell.rowKey,
      outputLabel: column ? localize(language, column.labelZh, column.labelEn) : cell.colKey,
      score: cell.score,
  ...
  return {
    grid: { left: 145, right: 48, top: 40, bottom: 95, containLabel: false },
    tooltip: {
      trigger: 'item',
...
function markChartReady(chart: ECharts, testId: string): void {
  const dom = chart.getDom();
  dom.dataset.phase2lChart = testId;
...
export const DependencyHeatmap: React.FC<DependencyHeatmapProps> = React.memo(({
  ...
  const selectedRow = selectedCell
    ? rows.find((row) => row.key === selectedCell.rowKey)
    : undefined;
  const selectedColumn = selectedCell
    ? columns.find((column) => column.key === selectedCell.colKey)
    : undefined;
  const curve = useMemo(
    () => selectedCell
  ...
  const handleCurveReady = useCallback((chart: ECharts) => {
    markChartReady(chart, 'dependency-curve');
  }, []);
      ...
      <div className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/40 md:grid-cols-[1fr_1fr_auto] md:items-end">
        <label className="space-y-1 text-xs font-semibold text-slate-600 dark:text-slate-300">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-indigo-500/30 dark:border-slate-700 dark:bg-slate-900"
          />
          <select
        ...
        <button
          ...
          onClick={() => exportPng(heatmapChartRef.current, 'dependency-local-sensitivity', theme)}
          className="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-xs font-bold text-white hover:bg-indigo-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/50"
        >
          <Download size={14} />
          {t('导出 PNG', 'Export PNG')}
        </button>
          ...
          {selectedCell && selectedRow && selectedColumn ? (
            <>
              <div data-testid="dependency-selection-summary" className="rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                  <div>
                    ...
                    <div className="font-mono text-lg font-black">
                      {selectedCellModel?.score === null || selectedCellModel === null
                        ? '—'
                        : `${selectedCellModel.score > 0 ? '+' : ''}${selectedCellModel.score.toFixed(2)}`}
                    </div>
                  </div>
                  <div>
                ...
                {selectedCellModel?.score !== null && selectedCellModel ? (
                  <div className="mt-2 flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300">
                    {selectedCellModel.score > 0 ? <TrendingUp size={13} /> : <TrendingDown size={13} />}
                    {selectedCellModel.score > 0
                  ...
                  <div className="mt-2 flex items-start gap-1.5 rounded-lg bg-slate-100 p-2 text-xs text-slate-600 dark:bg-slate-900 dark:text-slate-300">
                    <Info size={13} className="mt-0.5 shrink-0" />
                    {selectedCellModel?.reason ?? t('该关系没有可验证的有限数值。', 'This relationship has no verifiable finite value.')}
                  </div>
                )}
              </div>
