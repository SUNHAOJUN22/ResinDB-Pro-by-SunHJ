...
const EXCLUDED_PARTS = new Set([
  '.git', 'artifacts', 'build', 'dist', 'node_modules', 'coverage', '.vite', '.vitest',
]);
...
export function auditTrackedText() {
  ...
  for (const relative of files) {
    ...
    const controls = [...text]
      .filter((character) => {
        ...
        return value !== 0x09 && value !== 0x0a && ((value >= 0 && value <= 0x08) || (value >= 0x0b && value <= 0x1f) || value === 0x7f);
      })
      .map((character) => `U+${character.codePointAt(0).toString(16).toUpperCase().padStart(4, '0')}`);
    if (controls.length > 0) {
      failures.push({ path: relative, category: 'control_character', detail: [...new Set(controls)].sort().join(', ') });
    }
  ...
  failures.sort((left, right) => `${left.path}\0${left.category}\0${left.detail}`.localeCompare(`${right.path}\0${right.category}\0${right.detail}`));
  return {
