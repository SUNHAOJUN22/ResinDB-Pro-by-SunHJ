#!/usr/bin/env bash
set -uo pipefail
out=/tmp/resindb-final-current-tree-proof-20260726
logs="$out/logs"
rm -rf "$out"
mkdir -p "$logs"
overall=0
...
if '"scripts/final-current-tree-proof.sh"' not in text:
...
rm -f .github/workflows/final-current-tree-proof-20260726.yml
rm -f .github/final-current-tree-proof-20260726.trigger
rm -f scripts/final-current-tree-proof.sh
rm -f .github/workflows/fresh-lockfile-rebuild-20260726.yml
rm -f .github/fresh-lockfile-rebuild-20260726.trigger
rm -f .github/workflows/lockfile-rebuild-diagnostic-20260726.yml
rm -f .github/lockfile-rebuild-diagnostic-20260726.trigger
...
import re
from pathlib import Path
root = Path('/tmp/resindb-final-current-tree-proof-20260726')
logs = root / 'logs'
