...
import type { KMeansMessage, KMeansResponse } from '../lib/kmeansWorker';
...
export function useKMeansWorker() {
  ...
  useEffect(() => {
    const worker = new Worker(new URL('../lib/kmeansWorker.ts', import.meta.url), { type: 'module' });
    ...
    worker.onmessage = (e: MessageEvent<KMeansResponse>) => {
       ...
       if (res.type === 'KMEANS_RESULT') {
          ...
          console.error("KMeansWorker Error:", res.payload.message);
          setIsComputing(false);
       }
    };
