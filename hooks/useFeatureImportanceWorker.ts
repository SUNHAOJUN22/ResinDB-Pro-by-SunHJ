...
import type { FeatureImportanceMessage, FeatureImportanceResponse } from '../lib/featureImportanceWorker';
...
export function useFeatureImportanceWorker() {
  ...
  useEffect(() => {
    workerRef.current = new Worker(new URL('../lib/featureImportanceWorker.ts', import.meta.url), {
      type: 'module'
    });
