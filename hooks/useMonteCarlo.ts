...
import type { MonteCarloMessage, MonteCarloResponse } from '../lib/monteCarloWorker';
...
export function useMonteCarlo() {
  ...
  useEffect(() => {
    const worker = new Worker(new URL('../lib/monteCarloWorker.ts', import.meta.url), { type: 'module' });
    ...
    worker.onmessage = (e: MessageEvent<MonteCarloResponse>) => {
      ...
      if (res.type === 'SIMULATION_COMPLETE' && res.payload) {
        ...
        console.error("MonteCarlo Error:", res.error);
      }
    };
