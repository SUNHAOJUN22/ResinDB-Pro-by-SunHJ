...
- `actions/checkout@v6`;
- `actions/setup-node@v6` with Node.js 22 and npm caching;
- `actions/upload-artifact@v7` for install diagnostics, coverage and UI evidence.
...
The repository audit executed on Linux with Node.js `v22.23.1` and npm `10.9.8` and recorded zero exit codes for:
...
The baseline contains **9 test files and 79 passing tests**. Coverage from that run was:
...
These values describe one verified baseline; they are not permanent promises for later commits. The final repository-cleanup run is stored under `reports/final-validation-20260726/`.
...
UI screenshots are generated by the tested production preview in Chromium. Hand-authored illustrations in `docs/assets/` explain architecture and workflow but are **not** UI test evidence. CI evidence is uploaded from `artifacts/` and `coverage/` for the commit that produced it.
