...
- **Stack context:** React + Vite + deterministic Python SVG generation.
...
All structural icons are inline SVG paths generated by the same Python script:
...
`scripts/generate-readme-visuals.py` owns the complete `docs/assets/resindb-*.svg` inventory. A generated file is accepted only when the checked-in version is byte-for-byte identical to a clean regeneration.
